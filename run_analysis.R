...
### q <- sqldf ("select * from selected")  : this is working 

subject = 1
activity = 'WALKING'
field = "tBodyAccmeanX"
field1 = colnamesSelectedDf[1]
field1
q =  paste (c("select * from selected where Subject = ",subject),collapse="")
q
sqldf (q)

q1 = paste (c("select * from selected where Activity = ","'", activity,"'"),collapse="")
q1
sqldf (q1)

q2 = paste (c("select * from selected where Activity = ","'", activity,"'"," and Subject = ","'",subject,"'"),collapse="")
...
q3 = paste (c("select avg(tBodyAccmeanX) from selected where Activity = ","'", activity,"'"," and Subject = ","'",subject,"'"),collapse="")

q3

sqldf (q3)

q4 =  paste (c("select avg(", field, ") from selected where Activity = ","'", activity,"'"," and Subject = ","'",subject,"'"),collapse="")
q4
sqldf (q4)  ## good


q5 =  paste (c("select avg(", field1, ") from selected where Activity = ","'", activity,"'"," and Subject = ","'",subject,"'"),collapse="")
q5
sqldf (q5)  ## good 0.27738


## test for loop in subject

for (i in 1: 6)
 
{
  q6 =  paste (c("select avg(", field1, ") from selected where Activity = ","'", activity,"'"," and Subject = ","'",i,"'"),collapse="")
  q6
  result =  sqldf (q6)  ## good 0.27738
  print (result)
}  ## good for 0.2773308, 0.2764266, 0.2755675, 0.278582, 0.2778432, 0.2836589


## test for loop in subject and activity
## set countj = 0;
countj = 0
for (i in 1: 6)
  
{
  ...
  {
     q7 =  paste (c("select avg(", field1, ") from selected where Activity = ","'", j ,"'"," and Subject = ","'",i,"'"),collapse="")
     q7
     result7 =  sqldf (q7)  ## good 0.27738
     print (result7)
     countj = countj + 1
  }
}  ## good for
print (countj)  ### good count = 36, and result7 = some of 0.2755675, 0.278592, and 0.2836589


## test for loop in subject and activity and fiels
  ...
  
{
  for (j in subjectActivity)
  {
    ...
    {
      q8 =  paste (c("select avg(", myField, ") from selected where Activity = ","'", j ,"'"," and Subject = ","'",i,"'"),collapse="")
      q8
      result8 =  sqldf (q8)  
      print (result8)
...
print (countField)  ### good count = 1728, and result8 = some of -0.9471368 ,and -0.9635715




## append to a vector : append function store value back by reference; example a <- new vector ; a <- append (a,1)
## article http://stackoverflow.com/questions/14392075/cannot-create-an-empty-vector-and-append-new-elements-in-r
## append to df : first create empty data frame : ddf = data.frame()
## set number of column by: numColumns = 50
## create elements : ddf = data.frame (t(repA(NA,numColumns)))
## remove the NA elements : ddf = ddf [-1,]
## add element to data frame by index : ddf[1,x]
  ...
  
{
  for (j in subjectActivity)
  {
    ...
    for (myField in colnamesSelectedDf [1:48])
    {
      q9 =  paste (c("select avg(", myField, ") from selected where Activity = ","'", j ,"'"," and Subject = ","'",i,"'"),collapse="")
      q9
      result9 =  sqldf (q9)  
      ##print (result9)
      countTotalField = countTotalField + 1
      dfIndex = dfIndex + 1
      ddf [1,dfIndex] = result9
    }
    ## print out df index
    print (dfIndex)
...
## add data frame to bottom of another tidy data frame




ddfcolnames = c("Subject",
  "Activity",
  "tBodyAcc-mean()-X",
  "tBodyAcc-mean()-Y",
  "tBodyAcc-mean()-Z",
  ...
  
{
  for (j in subjectActivity)
  {
    ...
    for (myField in colnamesSelectedDf [1:48])
    {
      q10 =  paste (c("select avg(", myField, ") from selected where Activity = ","'", j ,"'"," and Subject = ","'",i,"'"),collapse="")
      q10
      result10 =  sqldf (q10)  
      ##print (result9)
      countTotalField = countTotalField + 1
      dfIndex = dfIndex + 1
      ddf [1,dfIndex] = result10
    }
    ## print out df index
    print (dfIndex)
    ## print out data frame
    names(ddf) = ddfcolnames
    print (ddf)
  }
}  ## good for
print (countTotalField)  ### good count = 1728, and result8 = some of -0.9471368 ,and -0.9635715

## beautiful col names of tidy based on selected big column name  ? have to look at it <<<<<<======================================================================
  ...
  
{
  for (j in subjectActivity)
  {
    ...
    for (myField in colnamesSelectedDf [1:48])
    {
      q11 =  paste (c("select avg(", myField, ") from selected where Activity = ","'", j ,"'"," and Subject = ","'",i,"'"),collapse="")
      q11
      result11 =  sqldf (q11)  
      ##print (result9)
      countTotalField = countTotalField + 1
      dfIndex = dfIndex + 1
      ddf [1,dfIndex] = result11
    }
    ## print out df index
    print (dfIndex)
    ## print out data frame
